# Distributed under the terms of the GNU General Public License v2

EAPI=6

inherit meson

DESCRIPTION="X.Org combined protocol headers"
HOMEPAGE="https://cgit.freedesktop.org/xorg/proto/xorgproto/"
if [ "${PV}" = "${PV%_p*}" ] ; then
	SRC_URI="https://xorg.freedesktop.org/archive/individual/proto/xorgproto-${PV}.tar.gz"
else
	inherit git-r3
	EGIT_REPO_URI="https://anongit.freedesktop.org/git/xorg/proto/xorgproto"
	PVCD="${PV#*_p}"
	if [ ${PVCD} -ge 19700101 ] && [ ${PVCD} -lt 30000000 ] ; then # Not y3k compliant, sorry!
		PVCDY="${PVCD%????}"
		PVCDMD="${PVCD#????}"
		PVCDM="${PVCDMD%??}"
		PVCDD="${PVCDMD#??}"
		EGIT_COMMIT_DATE="${PVCDY}-${PVCDM}-${PVCDD}"
	fi
fi

LICENSE="GPL-2 MIT"
SLOT="0"
KEYWORDS="*"
IUSE="legacy"
LEGACY_DEPS="
!<x11-proto/evieproto-1.1.1
!<x11-proto/fontcacheproto-0.1.3
!<x11-proto/lg3dproto-5.0
!<x11-proto/printproto-1.0.5
!<x11-proto/xcalibrateproto-0.1.0
!<x11-proto/xf86rushproto-1.2.2"
DEPEND="legacy? ( ${LEGACY_DEPS} )
!<x11-proto/applewmproto-1.4.2
!<x11-proto/bigreqsproto-1.1.2
!<x11-proto/compositeproto-0.4.2
!<x11-proto/damageproto-1.2.1
!<x11-proto/dmxproto-2.3.1
!<x11-proto/dri2proto-2.8
!<x11-proto/dri3proto-1.2
!<x11-proto/fixesproto-5.0
!<x11-proto/fontsproto-2.1.3
!<x11-proto/glproto-1.4.17
!<x11-proto/inputproto-2.3.2
!<x11-proto/kbproto-1.0.7
!<x11-proto/presentproto-1.2
!<x11-proto/randrproto-1.6.0
!<x11-proto/recordproto-1.14.2
!<x11-proto/renderproto-0.11.1
!<x11-proto/resourceproto-1.2.0
!<x11-proto/scrnsaverproto-1.2.2
!<x11-proto/trapproto-3.4.3
!<x11-proto/videoproto-2.3.3
!<x11-proto/windowswmproto-1.0.4
!<x11-proto/xcmiscproto-1.2.2
!<x11-proto/xextproto-7.3.0
!<x11-proto/xf86bigfontproto-1.2.0
!<x11-proto/xf86dgaproto-2.1
!<x11-proto/xf86driproto-2.1.1
!<x11-proto/xf86miscproto-0.9.3
!<x11-proto/xf86vidmodeproto-2.3.1
!<x11-proto/xineramaproto-1.2.1
!<x11-proto/xproto-7.0.32
!<x11-proto/xproxymngproto-1.0.3 "
